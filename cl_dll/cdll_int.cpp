...
int DLLEXPORT Initialize( cl_enginefunc_t *pEnginefuncs, int iVersion )
{
	...
	// Engine version for crash log (set before SetGameDir so the INIT header
	// it writes already carries the version strings)
	{
		char eVer[64] = {0};
	...
	// Set crash log path from game directory
	const char *gamedir = gEngfuncs.pfnGetGameDirectory();
	CrashHandler_SetGameDir(gamedir);
